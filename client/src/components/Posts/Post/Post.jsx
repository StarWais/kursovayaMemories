...
export default function Post({ post, posts, setPosts }) {
  ...
  const { authToken, userId, isAdmin, selectedPosts, deletePosts } =
    useContext(GlobalContext);
  const [authTokenValue] = authToken;
  const [isAdminValue] = isAdmin;
  const [selectedPostsValue, setSelectedPostsValue] = selectedPosts;
  ...
  useEffect(() => {
    if (checkboxSelected) {
      setSelectedPostsValue((prev) => [...prev, post._id]);
    } else {
      setSelectedPostsValue((prev) => prev.filter((p) => p !== post._id));
    }
  }, [checkboxSelected]);
  ...
  const deletePost = () => {
    if (selectedPostsValue.length === 0) {
      setLoading(true);
      axios
        .post(
  ...
  return (
    <Card className={classes.root} component={motion.div}>
                  ...
                  {comments.map((comment, id) => (
                    <div key={uuidv4()}>
                      <Grid container direction="row" alignItems="center">
                        ...
                        {(userIdValue === comment.userID || isAdminValue) && (
      ...
      <CardActions disableSpacing>
        ...
        {!loading && (post.creator === userIdValue || isAdminValue) && (
          <IconButton
            aria-label="delete"
            onClick={deletePost}
            component={motion.div}
