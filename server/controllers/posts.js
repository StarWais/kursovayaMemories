  ...
  if (req.query && Object.keys(req.query).length === 0) {
    ...
    try {
      if (req.query.withLikes === 'true') {
        ...
        if (req.query.likesCount !== '') {
          newPosts = newPosts.filter(
            (post) => post.likedUsers.length <= +req.query.likesCount
          );
        }
        if (req.query.liker !== '') {
          const liker = await User.findOne({ firstName: req.query.liker });
          newPosts = newPosts.filter((post) =>
            post.likedUsers.includes(liker._id)
          );
        }
        if (req.query.withComments === 'true') {
          ...
          if (req.query.commentText) {
            newPosts = newPosts.filter((post) =>
              post.comments.find((comment) =>
                comment.commentText
                  .toLowerCase()
                  .includes(req.query.commentText.toLowerCase())
              )
            );
          }
        }
...
export const deletePost = async (req, res) => {
  ...
  try {
    ...
    if (post) {
      if (post.creator === user._id || user.isAdmin) {
        await PostMessage.findByIdAndRemove(id);
        const newPosts = await PostMessage.find();
...
export const removeComment = async (req, res) => {
  ...
  try {
    await PostMessage.findById(postID, (err, post) => {
      ...
      if (comment.userID !== req.user._id && !user.isAdmin) {
        return res
          .status(409)
...
export const getMaxLikes = async (req, res) => {
  let maxLikes = 0;
  const posts = await PostMessage.find();
  posts.forEach((post) => {
    if (post.likedUsers.length > maxLikes) {
      maxLikes = post.likedUsers.length;
    }
  });
  return res.status(200).json({ maxLikes });
};
